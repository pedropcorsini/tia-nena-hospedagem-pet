        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {services.map((service) => {
            ...
            return (
              <article
                key={service.title}
                className="relative overflow-hidden rounded-[2rem] border border-ink/8 bg-cream p-6 shadow-sm transition-colors duration-200 hover:border-honey/70"
              >
                <div className="absolute -right-10 -top-10 h-28 w-28 rounded-full bg-honey/18" />
                <span className="relative flex h-14 w-14 items-center justify-center rounded-2xl bg-honey text-ink shadow-sm">
